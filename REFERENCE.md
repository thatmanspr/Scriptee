...
A vim-motion TUI screenwriter for Linux. This is the full usage/config
reference and dev changelog; see [`README.md`](README.md) for the short
...
| `:cover`      | (re-)fill in the cover page fields, prefilled with whatever's already set |
...
- **`general.pdf_scene_numbers`** — stamp scene numbers in the PDF's left
  ...
  PDF.
- **`general.cover_page`** — master on/off for the PDF cover page,
  independent of whatever's in the title-page fields (default `true`).
  Set `false` to never draw one on `:pdf`, full stop — `:cover` still
  works either way, so fields can be filled in ahead of flipping this
  back on later.
- **`general.prompt_missing_titlepage`** — if a script has no title page
  ...
  page (it only adds one `if cover_page and metadata:`). Leaving every
  field blank when asked is treated as "no cover page, on purpose" and
  isn't asked again for that file this session. Set `false` to never ask
  and just export without one. Has no effect if `cover_page` above is
  `false`.
...
regression tests for each bug fixed below. 226 tests total.
...
- **A `.fountain` file imported from elsewhere with no title page
  ...
  `:pdf`.
- **New `:cover` command** — re-opens that same prompt on demand,
  prefilled with whatever's already set (Enter keeps a field, clear it +
  Enter removes it), then drops back into the screenplay. Unlike `:pdf`'s
  own one-shot auto-prompt, `:cover` resets that one-shot state and shows
  the prompt every time it's run, however many times that is — for
  fixing a typo in the title, adding a contact line later, or clearing
  the cover page entirely without touching anything else.
- **New `general.cover_page` config flag** (default `true`) — a full
  master switch, independent of `general.prompt_missing_titlepage`
  above. `false` means `:pdf` never draws a cover page at all, even if
  every field is filled in; `:cover` still works either way, so fields
  can be set ahead of flipping this back on. `prompt_missing_titlepage`
  is only about whether the *prompt* fires — this is about whether the
  *page itself* ever gets drawn.
