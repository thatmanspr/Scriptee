...
echo "==> Installing bundled fonts (Courier Prime, for :pdf export)..."
FONTS_DIR="$HOME/.local/share/scriptee/fonts"
if [ -d "$SCRIPT_DIR/fonts" ]; then
    mkdir -p "$FONTS_DIR"
    cp "$SCRIPT_DIR"/fonts/CourierPrime-*.ttf "$FONTS_DIR/"
    echo "    Installed to $FONTS_DIR"
else
    echo "    No fonts/ directory found next to this script -- skipping."
    echo "    :pdf export will fall back to base-14 Courier (still works,"
    echo "    just denser-looking than Courier Prime at 12pt bold)."
fi
...
if [ ! -f ~/.config/scriptee/config.toml ]; then
    # Generated by calling the just-installed scriptee module's own
    # write_default_config() rather than cp'ing the checked-in
    # config.toml -- that's what lets it auto-detect the Courier Prime
    # fonts installed above (via _bundled_courier_prime()) and fill in
    # font_family/custom_font accordingly. A plain cp of the repo's
    # config.toml can't do that: it's a static file that has no idea
